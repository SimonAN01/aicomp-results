---
name: aicomp-results
description: 仅适配“无人机低空航拍图像语义分割”和“面向噪声标签数据的细粒度图像识别鲁棒微调”两个赛题，通过 HTTP 请求提交 ZIP 与作品名称、查询本次评分或读取公开排行榜。其他赛题尚未适配或验证。提交与个人查分需要用户提供 auth，公开排行榜无需登录。
---
...
提交、个人查分和公开排行榜功能均按上述两个赛题进行适配。
其他赛题尚未适配或验证，不能因网站、菜单或接口相同就假定兼容。
目标属于其他赛题时，说明当前适配范围；需要先单独核对字段、文件要求、
评分格式和排行榜接口，再进行适配，不能直接沿用现有提交流程。
...
认证、读取、文件比对与查分已完成真实请求验证。噪声标签细粒度识别赛题还完成了
真实 ZIP 上传、远端哈希比对、作品名称及文件保存、回读确认，并按新回执等待到
对应评分记录 `DONE` 后读回 `FS_`，完整流程已实测。
该次使用已有预测内容上传到新路径，未验证任意预测文件的内部格式。
分割赛题此前只验证了同文件跳过和查分。不要将某赛题的验证范围推广至所有赛题，
也不得把跳过重复提交或模拟测试描述成真实写入成功。
